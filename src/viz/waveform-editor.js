...
import { decodeAudio, encodeWav } from 'cdp-wasm/wav';
import { el, gemSelect, saveWavFile, canSaveFile, wavFileName } from '../ui/ui.js';
import {
  fit, zoomAt, zoomToRange, panByPixels, followStep, clampViewport, MIN_SPAN,
...
// The editor is a single full-screen overlay, so at most one is ever open. The
// app's bottom transport bar stays visible over it and needs to drive *this*
// audio rather than the patch's Output, so the live instance is published here.
let current = null;
export const activeWaveformEditor = () => current;
...
export function openWaveformEditor({ wav, audioCtx, name = 'audio', nodeId = null, onPromote = null, onCrop = null, player = null }) {
  ...
  // iOS Safari also reports a pinch as non-standard gesture events; the stage
  // handles the pinch itself below, so suppress the page-scaling default.
  ...
  let selAtDown = null;             // selection as it stood at pointerdown (restored if a pinch takes over)
  let startYaw = 0, startPitch = 0;
  let slop = 3;                     // click-vs-drag threshold, widened for fingers
  // Touch: live pointers by id, so a second finger can promote the gesture to a
  // pinch. One finger drags a selection / taps a cursor, two fingers zoom + pan.
  const pointers = new Map();
  let pinchStartDist = 0, pinchStartVp = null, pinchAnchor = 0;
  const pinchPair = () => { const it = pointers.values(); return [it.next().value, it.next().value]; };
  function beginPinch() {
    const [p, q] = pinchPair();
    // Undo whatever the first finger started — the gesture is a pinch, not a selection.
    if (drag === 'select' || drag === 'resize-a' || drag === 'resize-b') selection = selAtDown;
    drag = 'pinch'; moved = true;
    pinchStartVp = viewport;
    pinchStartDist = Math.max(1, Math.hypot(p.x - q.x, p.y - q.y));
    pinchAnchor = xToSample(viewport, (p.x + q.x) / 2, cssW);
    stage.style.cursor = 'grabbing';
    hoverX = hoverY = null; hover = null; renderReadout();
    paint();
  }
  function updatePinch() {
    const [p, q] = pinchPair();
    const dist = Math.max(1, Math.hypot(p.x - q.x, p.y - q.y));
    const midX = clamp((p.x + q.x) / 2, 0, cssW);
    // Keep the sample that was under the midpoint pinned to the midpoint, so the
    // pinch zooms *and* pans (dragging both fingers scrolls the view).
    const startSpan = pinchStartVp.endSample - pinchStartVp.startSample;
    const nextSpan = clamp(startSpan * (pinchStartDist / dist), MIN_SPAN, length);
    const start = pinchAnchor - (midX / Math.max(1, cssW)) * nextSpan;
    setViewport(clampViewport(
      { startSample: start, endSample: start + nextSpan, sampleRate, totalSamples: length }, length,
    ));
    markInteracting();
  }
  stage.addEventListener('pointerdown', (e) => {
    ...
    pointers.set(e.pointerId, { x: offsetX(e), y: offsetY(e) });
    if (pointers.size >= 2) { if (pointers.size === 2) beginPinch(); return; }
    const x = offsetX(e);
    const touch = e.pointerType === 'touch';
    slop = touch ? 8 : 3;             // fingers wobble; don't read a tap as a tiny drag
    downX = x; downY = offsetY(e); moved = false;
    hadSelectionAtDown = !!selection; selAtDown = selection; markInteracting();
    ...
    if (selection) {  // grab an edge to resize (touch targets get a wider grip)
      const grip = touch ? 14 : 6;
      const xa = sampleToX(viewport, selection.a, cssW), xb = sampleToX(viewport, selection.b, cssW);
      if (Math.abs(x - xa) <= grip) { drag = 'resize-a'; return; }
      if (Math.abs(x - xb) <= grip) { drag = 'resize-b'; return; }
    }
    drag = 'select'; selAnchor = downSample;
  });
  stage.addEventListener('pointermove', (e) => {
    const x = offsetX(e);
    if (pointers.has(e.pointerId)) pointers.set(e.pointerId, { x, y: offsetY(e) });
    if (drag === 'pinch') { if (pointers.size >= 2) updatePinch(); return; }
    if (drag && Math.abs(x - downX) > slop) moved = true;
    if (drag === 'orbit') {
    ...
    if (e.pointerType === 'touch') return;   // no crosshair without a hovering cursor
    updateHover(x, e);   // idle hover readout
  });
  stage.addEventListener('pointerup', (e) => {
    pointers.delete(e.pointerId);
    if (drag === 'pinch') {
      // Lifting one finger ends the pinch outright; the finger still down neither
      // resumes a selection nor lands as a tap.
      if (pointers.size < 2) { drag = null; stage.style.cursor = (spaceHeld || mode === 'waterfall') ? 'grab' : 'crosshair'; scheduleResume(); }
    ...
    if (e.pointerType === 'touch' && hover) { hover = null; hoverX = hoverY = null; renderReadout(); paint(); }
    scheduleResume();
  });
  // A cancelled pointer (OS gesture, palm rejection) must not leave a half-finished
  // drag armed — drop it and leave the view exactly as the last move left it.
  stage.addEventListener('pointercancel', (e) => {
    pointers.delete(e.pointerId);
    try { stage.releasePointerCapture(e.pointerId); } catch {}
    if (drag === 'select' && !moved) selection = selAtDown;
    if (pointers.size < 2) {
      drag = null; stage.style.cursor = (spaceHeld || mode === 'waterfall') ? 'grab' : 'crosshair';
      hoverX = hoverY = null; hover = null; renderReadout(); paint(); scheduleResume();
    }
  });
  ...
  const api = {
    close,
    // Play / stop this editor's audio — what its own ▶ button and Space do. The
    // bottom transport bar routes here while the editor is up.
    togglePlay,
    isPlaying: () => player.isPlaying(wav),
