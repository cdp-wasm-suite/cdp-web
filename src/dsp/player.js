...
// ---- output renderer (pluggable spatialisation stage) -----------------------
// The route from a decoded AudioBuffer to the speakers is a swappable renderer,
// so multichannel handling can evolve (e.g. binaural rendering of octophonic
// material) without touching any play() call site: setOutputRenderer(fn) with
// fn(audioCtx, sourceNode, buffer) — connect sourceNode to the destination
// however you like and optionally return a cleanup called on stop().
//
// The default: ask the device for the buffer's channel count (true multichannel
// playback on interfaces that have the outputs). When the device can't take
// them all, fold odd channels → L and even → R — WebAudio's >2ch interpretation
// is 'discrete', so without this an 8-channel sound would audition as just its
// first two channels on a stereo device. Saved WAVs are untouched either way.
function defaultOutputRenderer(audioCtx, src, buffer) {
  const nch = buffer.numberOfChannels;
  const dest = audioCtx.destination;
  if (nch > 2) {
    try { dest.channelCount = Math.min(nch, dest.maxChannelCount || 2); } catch {}
    if (dest.channelCount < nch) {
      const split = audioCtx.createChannelSplitter(nch);
      const merge = audioCtx.createChannelMerger(2);
      const g = audioCtx.createGain();
      g.gain.value = Math.sqrt(2 / nch);   // level-compensate the nch/2 sums per side
      src.connect(split);
      for (let c = 0; c < nch; c++) split.connect(merge, c, c % 2);
      merge.connect(g); g.connect(dest);
      return () => { try { split.disconnect(); merge.disconnect(); g.disconnect(); } catch {} };
    }
  }
  src.connect(dest);
}
let outputRenderer = defaultOutputRenderer;
export function setOutputRenderer(fn) { outputRenderer = fn || defaultOutputRenderer; }

export function createPlayer(audioCtx) {
  let src = null;            // live AudioBufferSourceNode | null
  let renderTail = null;     // renderer cleanup for the live voice | null
  ...
  function stop() {
    ...
    if (renderTail) { try { renderTail(); } catch {} renderTail = null; }
    if (token !== null) {
  ...
  async function play({ token: tk, buffer, sampleRate: sr, length: len, from = 0, loop: lp = false, loopRange: lr = null, origin: org = 'node' }) {
    ...
    s.buffer = buffer;
    renderTail = outputRenderer(audioCtx, s, buffer) || null;
