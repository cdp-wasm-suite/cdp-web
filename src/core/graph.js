...
import { EFFECTS, applyEffect, buildArgs, paramRange, programsFor, conformChannels, conformRate, decodeAudio, encodeWav, GENERATORS, genById, applyGenerator } from 'cdp-wasm';
...
// A normalised (0–1) envelope resolves against its parameter's range, so a node
// whose ranges are unlocked has to scale against the unlocked span — otherwise a
// curve drawn over the full engine range would render against the curated one.
// Mirrors paramRow's envRange() in ui.js; keep the two in step.
const envParam = (node, p) => (p && node.state?.unlocked
  ? { ...p, ...paramRange(p, node.state.values || {}, { unlocked: true }) }
  : p);

export const byId = Object.fromEntries(EFFECTS.map((e) => [e.id, e]));

// ---- model helpers ----------------------------------------------------------
// node = { id, type:'source'|'transform'|'rawTransform'|'pvocAnalyse'|'pvocResynth'|'output'|'log'|'pick'|'gather',
//          x, y, source?:{kind,wav}, effectId?, state?:{values,envs}, raw?:{program,args,data},
//          inPorts:[{name,kind,accepts?}], outPort:{name,kind}|null }
// edge = { id, from:{node,port}, to:{node,port}, kind }
// Port kinds: 'audio' | 'spectral' | 'breakpoint' | 'bank' (a set of sounds —
// {outputs:[wav bytes], names:[string]} — from a multi-output effect or Gather).
// Audio inputs accept any number of cables; the runner sums them implicitly at
// unity gain (see mixAudio). Other kinds are one cable per input.

export const inEdge = (patch, nodeId, port) =>
  patch.edges.find((e) => e.to.node === nodeId && e.to.port === port);
export const inEdges = (patch, nodeId, port) =>
  patch.edges.filter((e) => e.to.node === nodeId && e.to.port === port);
...
// Can this input port take a cable of `kind`? An in-port may declare
// accepts:[...] to take several kinds (e.g. Output takes audio or bank);
// without it, only its own kind matches.
export const portAccepts = (node, portName, kind) => {
  const p = (node.inPorts || []).find((x) => x.name === portName);
  if (p) return (p.accepts || [p.kind]).includes(kind);
  return portKind(node, portName) === kind;
};
...
// Implicit unity-gain mix for several cables landing on one audio input: conform
// sample rates to the first source, sum into max(channels) width (mono sources
// feed every channel; wider sources map channel-for-channel), length = the
// longest. Sources sum at unity like a DAW bus — the WAV is 32-bit float, so a
// hot sum doesn't clip in the file; balance upstream or with an explicit Mix
// node when unity isn't wanted.
function mixAudio(results) {
  const rate = decodeAudio(results[0].bytes).sampleRate;
  const decoded = results.map((r) => decodeAudio(conformRate(r.bytes, rate)));
  const nch = Math.max(...decoded.map((d) => d.channelData.length));
  const len = Math.max(...decoded.map((d) => d.channelData[0].length));
  const mixed = Array.from({ length: nch }, () => new Float32Array(len));
  for (const d of decoded) {
    const mono = d.channelData.length === 1;
    for (let c = 0; c < nch; c++) {
      const src = mono ? d.channelData[0] : d.channelData[c];
      if (!src) continue;
      const out = mixed[c];
      for (let i = 0; i < src.length; i++) out[i] += src[i];
    }
  }
  return { kind: 'audio', bytes: encodeWav({ sampleRate: rate, channelData: mixed }), dur: len / rate };
}
...
export function validateConnection(patch, from, to) {
  ...
  if (!portAccepts(tn, to.port, fk)) return { ok: false, reason: `type mismatch (${fk} → ${tk})` };
  const existing = inEdges(patch, to.node, to.port);
  if (existing.some((e) => e.from.node === from.node && e.from.port === from.port))
    return { ok: false, reason: 'already connected' };
  if (existing.length) {
    // Audio inputs take any number of cables — they sum implicitly at run time.
    // Everything else (spectral, breakpoint, bank) stays one cable per input.
    const kinds = existing.map((e) => portKind(patch.nodes.get(e.from.node), e.from.port));
    if (fk !== 'audio' || kinds.some((k) => k !== 'audio'))
...
export class GraphRunner {
  ...
  // Resolve the result at `outputId` by pulling its upstream graph. Memoised per
  // run (a fan-out source executes once). The graph is acyclic by construction
  // (validateConnection rejects cycles), so no cycle guard is needed here.
  // Returns the node result: {kind:'audio', bytes, dur} or
  // {kind:'bank', outputs:[Uint8Array], names:[string]} — callers branch on kind.
  async run(patch, outputId) {
    return this._resolve(patch, outputId, new Map());
  }
  ...
  async _exec(patch, id, memo) {
    ...
    const get = async (port) => {
      const list = inEdges(patch, id, port);
      if (!list.length) return null;
      const results = await Promise.all(list.map((e) => this._resolve(patch, e.from.node, memo)));
      if (results.length === 1) return results[0];
      // Several cables on one audio input → implicit unity-gain mix.
      if (results.some((r) => !r || r.kind !== 'audio'))
        throw new Error('only audio inputs can take several cables');
      log(`mix: ${results.length} sounds at ${node.type === 'transform' ? (byId[node.effectId]?.label || node.type) : node.type} · ${port}`);
      return mixAudio(results);
    };
    switch (node.type) {
      case 'source':
      ...
      case 'transform': {
        ...
        const a = await get('in'), b = await get('in2'), bank = await get('bank');
        const brk = await this._gatherBrk(patch, node, eff, (a && a.dur) || 1, memo);
        // pipeline effects (pitch octmove/altharms) are audio-in → audio-out
        // despite domain:'spectral': applyEffect's pipeline branch does its own
        // pvoc analysis internally, so they run through the sound path.
        return eff.domain === 'spectral' && !eff.pipeline ? this._transformSpectral(node, eff, a, b, brk) : this._transformSound(node, eff, a, b, brk, bank);
      }
      case 'pick': {
        const i = await get('in');
        if (!i || i.kind !== 'bank') throw new Error('Pick needs a bank input (a multi-output effect)');
        if (!i.outputs.length) throw new Error('Pick: the bank is empty');
        const idx = Math.max(0, Math.min(i.outputs.length - 1, (Number(node.state?.values?.index) || 1) - 1));
        const bytes = i.outputs[idx];
        return { kind: 'audio', bytes, dur: wavDuration(bytes) };
      }
      case 'gather': {
        const outputs = [], names = [];
        for (const p of node.inPorts || []) {
          const r = await get(p.name);
          if (r && r.kind === 'audio') { outputs.push(r.bytes); names.push(`${p.name}.wav`); }
        }
        if (!outputs.length) throw new Error('Gather has no connected sounds');
        return { kind: 'bank', outputs, names };
      }
  ...
  async _gatherBrk(patch, node, eff, dur, memo) {
    ...
    for (const port of node.paramPorts || []) {
      ...
      if (p) out[name] = envToBrk(r.pts, envParam(node, p), dur, r);
    }
    return out;
  }
  ...
  _mergeBrk(node, params, dur, connectedBrk) {
    ...
    for (const [k, e] of Object.entries(node.state?.envs || {})) {
      ...
      brk[k] = envToBrk(parseBrk(e.text), envParam(node, p), dur, e);
    }
  ...
  // mono-split / derive / data / pipeline / two-input / mixfile-chain internally).
  // Multi-input effects (variadicInputs / mixChain.multiInput) can take a bank on
  // their 'bank' port: with 'in' also cabled the whole bank becomes extra.inputs;
  // bank-only wiring uses leaf 0 as the main input and the rest as extras, so
  // e.g. partition → rejoin works over a single cable. multiOut effects return a
  // bank instead of audio.
  async _transformSound(node, eff, inp, inp2, connectedBrk, bank) {
    const takesBank = eff.variadicInputs || (eff.mixChain && eff.mixChain.multiInput);
    const bankWavs = (takesBank && bank && bank.kind === 'bank') ? bank.outputs : [];
    const extra = {};
    if (!inp && bankWavs.length) {
      inp = { bytes: bankWavs[0], dur: wavDuration(bankWavs[0]) };
      extra.inputs = bankWavs.slice(1);
    } else if (bankWavs.length) {
      extra.inputs = bankWavs;
    }
    ...
    const out = await applyEffect(this.cdp, eff, node.state.values || {}, inp.bytes, extra);
    if (eff.multiOut) return { kind: 'bank', outputs: out.outputs, names: out.names };
    // dur from the actual output: effects like tangent/crumble set their own length
    return { kind: 'audio', bytes: out, dur: wavDuration(out) };
  }
