...
const isSpectral = (fx) => byId[fx]?.domain === 'spectral' && !byId[fx]?.pipeline;
...
export const RECIPES = [
  ...
  chain({ name: 'Gritty saw', category: 'Distortion',
    ...
    gen('wave', { shape: '4', freq: 110, dur: 2 }),
  ...
  // ---- mix ------------------------------------------------------------------
  // Implicit mixing: both tones cable straight into the Output's input — audio
  // inputs sum any number of cables, no Mix node needed.
  {
    app: 'cdp-web-patch', v: 1,
    metadata: { name: 'Layered fifths', category: 'Mix',
      description: 'Two tones a fifth apart, both cabled into the same Output input — audio inputs mix implicitly, so layering needs no Mix node. (Use a Mix two sounds node when you want per-source levels.)' },
    nodes: [
      sourceNode('n1', 40, 80, gen('wave', { shape: '1', freq: 220, dur: 2, amp: 0.4 })),
      sourceNode('n2', 40, 300, gen('wave', { shape: '1', freq: 330, dur: 2, amp: 0.4 })),
      { id: 'n3', type: 'output', x: 340, y: 190 },
    ],
    edges: [
      { from: { node: 'n1', port: 'out' }, to: { node: 'n3', port: 'in' } },
  ...
  // ---- banks & multichannel ---------------------------------------------------
  // Partition deals the speech out into 3 interleaved waveset files (a bank on
  // one ▣ cable); rejoin remixes the whole bank back into a single sound.
  {
    app: 'cdp-web-patch', v: 1,
    metadata: { name: 'Partition & rejoin', category: 'Banks',
      description: 'Split the speech into 3 interleaved waveset files — a bank of sounds on one cable — then remix the whole bank back together with rejoin. Wire a Pick node to the ▣ cable to audition or process a single piece.' },
    nodes: [
      sourceNode('n1', 40, 150, src('speech')),
      { id: 'n2', type: 'transform', x: 260, y: 150, effectId: 'partition.wavesets', state: { values: { pieces: 3, group: 16 }, envs: {} } },
      { id: 'n3', type: 'transform', x: 500, y: 150, effectId: 'rejoin.rejoin', state: { values: { gain: 1 }, envs: {} } },
      { id: 'n4', type: 'output', x: 730, y: 150 },
    ],
    edges: [
      { from: { node: 'n1', port: 'out' }, to: { node: 'n2', port: 'in' } },
      { from: { node: 'n2', port: 'out' }, to: { node: 'n3', port: 'bank' } },
      { from: { node: 'n3', port: 'out' }, to: { node: 'n4', port: 'in' } },
    ],
  },

  // Gather collects three synth tones into a bank; multimix.spread gives each
  // its own output channel (a 3-channel file).
  {
    app: 'cdp-web-patch', v: 1,
    metadata: { name: 'Gather to channels', category: 'Banks',
      description: 'Three tones collected into a bank by a Gather node, then multimix places each on its own output channel — a 3-channel file from three mono sources. Add more Gather inputs (+ input) for wider spreads.' },
    nodes: [
      sourceNode('n1', 40, 60, gen('wave', { shape: '1', freq: 220, dur: 3, amp: 0.6 })),
      sourceNode('n2', 40, 250, gen('wave', { shape: '2', freq: 277.2, dur: 3, amp: 0.5 })),
      sourceNode('n3', 40, 440, gen('wave', { shape: '3', freq: 329.6, dur: 3, amp: 0.4 })),
      { id: 'n4', type: 'gather', x: 300, y: 250, gather: { inputs: 3 } },
      { id: 'n5', type: 'transform', x: 520, y: 250, effectId: 'multimix.spread', state: { values: {}, envs: {} } },
      { id: 'n6', type: 'output', x: 750, y: 250 },
    ],
    edges: [
      { from: { node: 'n1', port: 'out' }, to: { node: 'n4', port: 'in1' } },
      { from: { node: 'n2', port: 'out' }, to: { node: 'n4', port: 'in2' } },
      { from: { node: 'n3', port: 'out' }, to: { node: 'n4', port: 'in3' } },
      { from: { node: 'n4', port: 'out' }, to: { node: 'n5', port: 'bank' } },
      { from: { node: 'n5', port: 'out' }, to: { node: 'n6', port: 'in' } },
    ],
  },

  // Split a stereo sound into L/R, process each side differently, and
  // re-interleave — the bank cable carries both channels, Picks take one each.
  {
    app: 'cdp-web-patch', v: 1,
    metadata: { name: 'Stereo split-process', category: 'Banks',
      description: 'Split channels turns a stereo sound into a bank of two mono files; a Pick takes each side (item 1 = left, item 2 = right), the left runs through a resonant high-pass while the right stays dry, and Join channels interleaves them back into stereo.' },
    nodes: [
      sourceNode('n1', 40, 190, gen('wave', { shape: '2', freq: 165, dur: 3, amp: 0.6 })),
      { id: 'n2', type: 'transform', x: 250, y: 190, effectId: 'modify.space', state: { values: { pan: 0.4 }, envs: {} } },
      { id: 'n3', type: 'transform', x: 460, y: 190, effectId: 'housekeep.split', state: { values: {}, envs: {} } },
      { id: 'n4', type: 'pick', x: 670, y: 80, pick: { values: { index: 1 } } },
      { id: 'n5', type: 'pick', x: 670, y: 300, pick: { values: { index: 2 } } },
      { id: 'n6', type: 'transform', x: 880, y: 80, effectId: 'filter.resonhi', state: { values: { freq: 1200, acuity: 0.05, gain: 1 }, envs: {} } },
      { id: 'n7', type: 'transform', x: 1100, y: 190, effectId: 'submix.interleave', state: { values: {}, envs: {} } },
      { id: 'n8', type: 'output', x: 1330, y: 190 },
    ],
    edges: [
      { from: { node: 'n1', port: 'out' }, to: { node: 'n2', port: 'in' } },
      { from: { node: 'n2', port: 'out' }, to: { node: 'n3', port: 'in' } },
      { from: { node: 'n3', port: 'out' }, to: { node: 'n4', port: 'in' } },
      { from: { node: 'n3', port: 'out' }, to: { node: 'n5', port: 'in' } },
      { from: { node: 'n4', port: 'out' }, to: { node: 'n6', port: 'in' } },
      { from: { node: 'n6', port: 'out' }, to: { node: 'n7', port: 'in' } },
      { from: { node: 'n5', port: 'out' }, to: { node: 'n7', port: 'in2' } },
      { from: { node: 'n7', port: 'out' }, to: { node: 'n8', port: 'in' } },
    ],
  },

  // crumble scatters a mono sound around an 8-channel ring — saved WAVs keep all
  // 8 channels; stereo playback folds them down for auditioning.
  chain({ name: 'Crumble ring (8ch)', category: 'Banks',
    description: 'The marimba, iterated into a longer texture, projected onto an 8-channel ring and progressively "crumbled" into scattered segments. The saved WAV keeps all 8 channels; on stereo monitoring the audition is folded down.' },
    src('marimba'),
    [{ fx: 'extend.iterate', values: { reps: 6, pshift: 2, rand: 0.4 } },
     { fx: 'crumble.crumble', values: { start: 0.3, dur1: 2, dur2: 3, segsize: 0.15, segrand: 0.4, pitchvar: 3 } }]),
];
