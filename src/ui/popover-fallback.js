...
export function installPopoverFallback() {
  ...
  // Native popovers nest through their invoker (popoverTargetElement): a menu
  // whose trigger button sits inside an open modal belongs to that modal's
  // stack. Walk the invoker chain to ask "does `el` ultimately hang off `o`?".
  const nestedUnder = (el, o) => {
    for (let inv = el.__poInvoker; inv; inv = inv.closest('[popover]')?.__poInvoker)
      if (o.contains(inv)) return true;
    return false;
  };
  ...
  const show = (el) => {
    ...
    if (el.getAttribute('popover') !== 'manual') {
      for (const o of [...autos]) if (!o.contains(el) && !el.contains(o) && !nestedUnder(el, o)) hide(o);
      autos.add(el);
    }
  ...
  // invoker is left to its click toggle (or it would close-then-reopen). A press
  // inside a popover nested under `o` (via its invoker chain) keeps `o` open.
  document.addEventListener('pointerdown', (e) => {
    for (const o of [...autos]) {
      if (o.contains(e.target)) continue;
      if (o.__poInvoker && o.__poInvoker.contains(e.target)) continue;
      const pop = e.target.closest?.('[popover]');
      if (pop && pop !== o && nestedUnder(pop, o)) continue;
      hide(o);
    }
  }, true);
