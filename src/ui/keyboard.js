...
export function createKeyboard(target, opts = {}) {
  ...
  // Narrow screens can't fit the bar's controls side by side — they wrapped their
  // labels onto two lines — so CSS hides them there and shows this instead. The
  // items are built on open, so their check marks track the live state. Octave is
  // display-only in the bar (z / x on a hardware keyboard); the menu is the only
  // way to shift it by touch.
  const menuBtn = el('button', { class: 'kbd-btn kbd-menu', type: 'button', textContent: '⋯', title: 'Sampler controls' });
  dropdown(menuBtn, () => {
    const items = [{ label: 'ADSR', checked: adsrOpen, action: () => setAdsrOpen(!adsrOpen) }];
    ...
    items.push({ sep: true }, { group: `OCTAVE ${cfg.octave}` },
      ...
      { label: 'Octave +', disabled: cfg.octave >= 9, action: () => setOctave(cfg.octave + 1) },
      { sep: true }, { label: 'All notes off', action: releaseAll });
    return items;
  });
  const bar = el('div', { class: 'kbd-bar' }, title, adsrToggle, repitchToggle, menuBtn,
    el('span', { class: 'kbd-spacer' }), octLabel, panic, volWrap, closeBtn);
  ...
  function setOctave(o) {
    cfg.octave = clamp(Math.round(o), 0, 9);
    octLabel.textContent = `OCT ${cfg.octave}`;
  }
  function releaseAll() {
    target.allNotesOff();
  ...
  function onKeyDown(e) {
    ...
    if (key === 'z') { e.preventDefault(); setOctave(cfg.octave - 1); return; }
    if (key === 'x') { e.preventDefault(); setOctave(cfg.octave + 1); return; }
