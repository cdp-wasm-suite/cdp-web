...
/**
 * Is this 32-bit float WAV in the normalised sample domain a float file is meant
 * to carry? A few CDP programs accept the float-output flag but keep writing
 * values scaled to the 16-bit integer range (or leave NaNs behind), which is
 * only detectable from the samples. Non-float files pass unexamined — there is
 * nothing to be wrong about. Scans the data chunk in place, no allocation.
 *
 * @param {Uint8Array} bytes
 * @param {number} [limit]  largest plausible magnitude; CDP output can exceed
 *   1.0 legitimately (mixing, gain), but never by this much.
 * @returns {boolean}
 */
export function floatWavIsSane(bytes, limit = 100) {
  const dv = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
  if (dv.byteLength < 12 || str(dv, 0, 4) !== 'RIFF' || str(dv, 8, 4) !== 'WAVE') return true;
  let isFloat = false;
  let pos = 12;
  while (pos + 8 <= dv.byteLength) {
    const id = str(dv, pos, 4);
    ...
    if (id === 'fmt ') {
      let tag = dv.getUint16(body, true);
      if (tag === FMT_EXTENSIBLE && size >= 24) tag = dv.getUint16(body + 24, true);
      isFloat = tag === FMT_FLOAT && dv.getUint16(body + 14, true) === 32;
    } else if (id === 'data') {
      if (!isFloat) return true; // fmt precedes data in every file CDP writes
      const end = Math.min(body + size, dv.byteLength) - 3;
      for (let o = body; o < end; o += 4) {
        const v = dv.getFloat32(o, true);
        if (!(Math.abs(v) <= limit)) return false; // catches NaN too
      }
      return true;
    }
