...
// DOM stub must be the first import — see scripts/dom-stub.mjs.
import './dom-stub.mjs';

import { readFileSync } from 'node:fs';
import { fileURLToPath } from 'node:url';
import { dirname, resolve } from 'node:path';
import { CDP, EFFECTS, ENVELOPE_PARAMS, decodeAudio } from 'cdp-wasm';
...
for (const recipe of RECIPES) {
  ...
  try {
    ...
    const res = await runner.run(patch, out.id);
    if (res.kind === 'bank') {
      // A recipe ending in a bank: every leaf must decode to non-empty audio.
      if (!res.outputs.length) throw new Error('empty bank');
      for (const leaf of res.outputs) {
        const a = decodeAudio(leaf);
        if (!leaf?.length || !(a.channelData?.[0]?.length > 0)) throw new Error('empty bank item');
      }
      console.log(`✓ ${name.padEnd(26)} bank of ${res.outputs.length}`);
      continue;
    }
