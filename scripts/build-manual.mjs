...
if (existsSync(appLog))
  pages['release-notes'] = { title: 'cdp-web release notes', html: convert(readFileSync(appLog, 'utf8'), 'CHANGELOG.md') };
const engineLog = [
  join(root, 'node_modules/cdp-wasm/CHANGELOG.md'),
  join(root, 'vendor/cdp-wasm/CHANGELOG.md'),
].find((p) => existsSync(p));
if (engineLog)
  pages['release-notes-engine'] = { title: 'cdp-wasm engine release notes', html: convert(readFileSync(engineLog, 'utf8'), 'CHANGELOG.md') };

// ---- nav sidebar -------------------------------------------------------------
const has = (slug) => Object.prototype.hasOwnProperty.call(pages, slug);
const entry = (slug, label) => ({ slug, label: label || pages[slug].title });

const GUIDE_ORDER = ['guide/README', 'guide/spectral-pvoc', 'guide/breakpoints', 'guide/mono-and-channels', 'guide/banks-and-multichannel'];
